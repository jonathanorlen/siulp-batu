...
{   
    ...
    //$this->dompdf->render();
    //$this->dompdf->stream("surat_perintah_tugas_$id.pdf",array('Attachment'=>0));
...
public function simpan_tambah() {
    ...
    if($cek_hps=='cukup'){
        ...
        $data['no_rup'] = $this->input->post("no_rup");
...
// $tanggal_rencana_pengadaan = $get_tanggal_rencana_pengadaan->row();
        ...
        if($get_group->kode_jabatan=="007"){
            $kode_ppk = $this->input->post('kode_ppk');
            $data['kode_ppk'] = $kode_ppk;
            $get_ppk = $this->db->get_where('master_user',array('nip'=>$kode_ppk));
            $hasil_ppk = $get_ppk->row();
            $data['nama_ppk'] =$hasil_ppk->nama;
...
public function simpan_ubah_item() {
    ...
    if(!empty($data['nama_pokja'])){
        $hasil_dana = $this->db->get_where('master_kelompok_kerja',array('pokja' => @$data['nama_pokja']));
        $hasil_data_dana = $hasil_dana->row();
        $data['kode_pokja'] = @$hasil_data_dana->id;
    }
...
public function form_jasa_konsultasi()
{
    $data['konten'] = $this->load->view('create_paket/form_jasa_konsultasi', NULL, TRUE);
    $this->load->view ('main', $data);
}
public function form_konstruksi()
{   
    $data['konten'] = $this->load->view('create_paket/form_konstruksi', NULL, TRUE);
    $this->load->view ('main', $data);
}
public function form_barang()
{   
    $data['konten'] = $this->load->view('create_paket/form_barang', NULL, TRUE);
    $this->load->view ('main', $data);
}

public function simpan_form_konstruksi()
{   

    $id = $this->input->post("id");
    $im_hardcopy = implode("|", $this->input->post('hardcopy'));
    $data['hardcopy'] = $im_hardcopy;

    $im_softcopy = implode("|", $this->input->post('softcopy'));
    $data['softcopy'] = $im_softcopy;

    $this->db->update('transaksi_data_paket',$data,array("id" => $id));

}
public function simpan_form_barang()
{   
...
public function simpan_form_jasa_konsultasi()
{   
...
public function print_konstruksi()
{
    $this->load->view('create_paket/print_form_konstruksi');
}

public function print_jasa_konsultasi()
{
    $this->load->view('create_paket/print_jasa_konsultasi');
}

public function print_barang()
{
    $this->load->view('create_paket/print_barang');
}
...
public function get_ppk(){
    $data = $this->input->post();
    $this->db->where('kode_skpd',$data['kode_ppk']);
    $barang = $this->db->get('master_user');
    $hasil_barang = $barang->result();
            #echo $this->db->last_query();
    foreach($hasil_barang as $daftar){
        echo "<option value='$daftar->nip'>$daftar->nama</option>";
    }
}
...
public function get_skpd(){
    $data = $this->input->post();
    $this->db->where('kode_skpd',$data['kode_ppk']);
    $skpd = $this->db->get('master_satuan_kerja');
    $hasil_skpd = $skpd->row();
    echo $hasil_skpd->unit_kerja;

}

public function simpan_ubah_skpd(){
    $data = $this->input->post();
    $get_nama_skpd = $this->db->get_where('master_satuan_kerja',array('kode_skpd'=>$data['kode_skpd']));
    $hasil_skpd = $get_nama_skpd->row();
    $get_nama_ppk = $this->db->get_where('master_user',array('nip'=>$data['kode_ppk']));
    $hasil_nama_ppk = $get_nama_ppk->row();
    
    $update['kode_satuan_kerja'] = $data['kode_skpd'];
    $update['nama_satuan_kerja'] = $hasil_skpd->unit_kerja;
    $update['kode_ppk'] = $data['kode_ppk'];
    $update['nama_ppk'] = $hasil_nama_ppk->nama;
    $this->db->update('transaksi_data_paket',$update,array('id'=>$data['id']));
}

public function simpan_ubah_status(){
    $data = $this->input->post();
    $ubah['status_paket'] = $data['status_paket'];
    
    $this->db->update('transaksi_data_paket',$ubah,array('id'=>$data['id']));
}
