...
class rapat extends MY_Controller {
	...
	{	
		...
		$data_verifikasi['nomor_sprpp'] = $this->input->post("nomor");
		...
		$data_verifikasi['status_paket'] = 'Proses Lelang';
...
//		echo $this->db->last_query();
	...
	{	
		$data['menu'] = 'master';
		$data['konten'] = $this->load->view('rapat/rapat/form_kirim_notulensi', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	{	
		...
		@$spesifikasi = implode("|", $data['spesifikasi_teknis']);
		@$simpan['spesifikasi_teknis'] =@$spesifikasi;

		$simpan['nama'] = $data['nama'];
		$simpan['jabatan'] = $data['jabatan'];
		$simpan['cv'] = $data['cv'];
...
//		echo $this->db->last_query();
	...
	{	
		...
		$simpan['kode_rapat'] = $data['kode_rapat'];
		$simpan['kode_paket'] = $data['kode_paket'];
		$simpan['nama'] = $data['nama'];
		$simpan['jabatan'] = $data['jabatan'];
		$simpan['cv'] = $data['cv'];
		$simpan['spt_tahunan'] = $data['spt'];
		...
		$this->db->insert("record_notulensi",$simpan);

		$simpan_rapat['tanggal_notulensi']=date("Y-m-d");
		$simpan_rapat['notulensi']='terisi';
		$this->db->update("transaksi_rapat",$simpan_rapat,array('id'=> $id));
//		echo $this->db->last_query();
...
//		echo $this->db->last_query();
	...
	public function simpan_notulensi()
	{	
		$data = $this->input->post();
		$id = $data['id'];
		$simpan['biaya_penunjang'] = $data['biaya_penunjang'];
...
//		echo $this->db->last_query();
	...
	{	
		...
		$simpan_notulensi['notulensi']='terisi';
		$simpan_notulensi['tanggal_notulensi']= date("Y-m-d");

		$this->db->insert("record_notulensi",$simpan);
		$this->db->update("transaksi_rapat", $simpan_notulensi,array('id'=>$id));
//		echo $this->db->last_query();
