...
class data_verifikasi extends MY_Controller {
	...
	{	
		$data['menu'] = 'Data';
		$data['konten'] = $this->load->view('data_verifikasi/content', NULL, TRUE);
	...
	{	
		...
		$data['status_paket'] ="Revisi Dokumen";
		$data['posisi'] ="Revisi Dokumen";
	...
	{	
		...
		$kode= $this->input->post("id");
		$data_verifikasi['kode_pokja'] = $this->input->post('kode_paket');
		$hasil = $this->db->get_where('master_kelompok_kerja',array('id' =>$data_verifikasi['kode_pokja']));
		$hasil_data = $hasil->row();
		$data_verifikasi['nama_pokja'] = $hasil_data->pokja;

		$data_verifikasi['status_paket'] ="Lolos Verifikasi";
		$data_verifikasi['posisi'] ="proses penjadwalan";
		$data_verifikasi['status_surat'] ="terbit surat";
		...
		public function simpan_terima2()
	{	
		...
		$id  = $user->id;
		$nama = $user->nama;
		$data_verifikasi['kode_sekretariat'] = $id;
		$data_verifikasi['nama_sekretariat'] = $nama; 
		...
		$data_verifikasi['kode_rapat'] = $kode;

		$data_verifikasi['kode_paket'] = $this->input->post('kode_paket');
		$data_verifikasi['nama_paket'] = $this->input->post('paket');

		$hasil_pokja = $this->db->get_where('transaksi_data_paket',array('kode_paket' => $this->input->post('kode_paket')));
		$data_pokja = $hasil_pokja->row();	

		$data_verifikasi['nama_pokja'] = $data_pokja->nama_pokja;
		$data_verifikasi['kode_pokja'] = $data_pokja->kode_pokja;
		$data_verifikasi['kode_ppk'] = $this->input->post('kode_ppk');
		$data_verifikasi['nama_ppk'] = $this->input->post('nama_ppk');
		$data_verifikasi['nama_satuan_kerja'] = $this->input->post('nama_satuan_kerja');
		$data_verifikasi['kode_satuan_kerja'] = $this->input->post('kode_satuan_kerja');
		$data_verifikasi['agenda'] = $this->input->post('agenda');
		$data_verifikasi['tempat'] = $this->input->post('tempat');
		$data_verifikasi['tanggal_rapat'] = $this->input->post('tanggal_rapat');
		$data_verifikasi['waktu'] = $this->input->post('waktu');
		$data_verifikasi['status_pokja'] = 'Proses Verifikasi';
		$data_verifikasi['status_ppk'] = 'Proses Verifikasi';
		$data_verifikasi['tahun'] = date('Y');
		$data_verifikasi['tutup_tahun'] = 'Open';

		
		$update=$this->db->insert("transaksi_rapat",$data_verifikasi);
	...
	{
		$data['konten'] = $this->load->view('data_verifikasi/verifikasi', NULL, TRUE);
		$this->load->view ('main', $data);
	}
