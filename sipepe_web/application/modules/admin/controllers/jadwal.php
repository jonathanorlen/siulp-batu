...
class jadwal extends MY_Controller {
	...
	{
		$data['konten'] = $this->load->view('jadwal/tambah', NULL, TRUE);
	...
	{
		//$item = 
		$data['konten'] = $this->load->view('jadwal/daftar', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	{
		$data['konten'] = $this->load->view('jadwal/ubah', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	public function simpan_ubah() {
       ...
       ;
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['judul'] 		= $this->input->post("judul");
			$data['isi'] = $this->input->post("isi_jadwal"); 
			$data['status'] = $this->input->post("status"); 
			$id 			    = $this->input->post("id");
           
            
			$get_old_file = $this->db->query("SELECT * FROM jadwal_kegiatan where id='$id'");
			...
			//$data['kategori_id'] = $this->input->post("kategori");
			$this->db->update("jadwal_kegiatan", $data, "id = " . $id);
	...
	public function hapus() {      
		$key = $_REQUEST['key'];
		$this->db->delete('jadwal_kegiatan', array('id' => $key));
		echo '<div class="alert alert-success">jadwal sudah dihapus.</div>';    
	}
}