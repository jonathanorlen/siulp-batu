...
class layanan extends MY_Controller {
	...
	{
		$data['konten'] = $this->load->view('layanan/tambah', NULL, TRUE);
	...
	{
		//$item = 
		$data['konten'] = $this->load->view('layanan/daftar', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	{
		$data['konten'] = $this->load->view('layanan/ubah', NULL, TRUE);
		$this->load->view ('main', $data);
	}
	...
	public function simpan_ubah() {
       ...
       ;
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['judul'] 		= $this->input->post("judul");
			$data['isi'] = $this->input->post("isi_layanan"); 
			$data['status'] = $this->input->post("status"); 
			$id 			    = $this->input->post("id");
           
            
			$get_old_file = $this->db->query("SELECT * FROM layanan where id='$id'");
	...
	public function hapus() {      
		$key = $_REQUEST['key'];
		$this->db->delete('layanan', array('id' => $key));
		echo '<div class="alert alert-success">layanan sudah dihapus.</div>';    
	}
}